...
import { multicolormediumlogo, multicolorrightlogo, servicelogo1, starrating } from '../assets'
    ...
    return (
        <Pressable onPress={() => setState(index)} style={{ height: HEIGHT * 0.2, width: WIDTH * 0.9, backgroundColor: '#181D23', marginTop: HEIGHT * 0.02, borderRadius: HEIGHT * 0.01 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', margin: HEIGHT * 0.02 }}>
                ...
                <View style={{ height: HEIGHT * 0.025, width: WIDTH * 0.055, borderWidth: 1, borderColor: '#0FC1A1', borderRadius: HEIGHT * 0.03, marginTop: HEIGHT * 0.006, backgroundColor: state === index ? '#0FC1A1' : '#181D23', justifyContent: 'center', alignItems: 'center' }}>{state === index ? <Ionicons name='checkmark' /> : ''}
                </View>
