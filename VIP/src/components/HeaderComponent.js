...
const HeaderComponent = ({ leftlogo, rightlogo, leftOnPress, title }) => {
    return (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingLeft: WIDTH * 0.065, paddingRight: WIDTH * 0.065, }}>
            <Pressable onPress={() => leftOnPress()}>
                <Image source={leftlogo} />
            </Pressable>
            <View style={{ marginRight: !rightlogo && WIDTH * 0.06 }}>
                <Text style={{ color: "#FFFFFF", fontSize: HEIGHT * 0.0205, textAlign: 'center', fontWeight: '400' }}>{title}</Text>
            </View>
            <Pressable>
