...
const ButtonComponent = ({ label, onPressFunction, Width }) => {
    return (
        <Pressable style={{ width: Width ? WIDTH * 0.55 : WIDTH * 0.85, borderRadius: HEIGHT * 0.01 }} onPress={() => onPressFunction()}>
            <View style={{ backgroundColor: '#0FC1A1', height: HEIGHT * 0.07, width: Width ? WIDTH * 0.55 : WIDTH * 0.85, borderRadius: HEIGHT * 0.01, justifyContent: 'center', alignItems: 'center' }}>
