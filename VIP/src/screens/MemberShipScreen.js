    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'black' }}>
            <View style={{ backgroundColor: '#181D23', height: HEIGHT * 0.43, width: WIDTH, borderBottomLeftRadius: HEIGHT * 0.01, borderBottomRightRadius: HEIGHT * 0.01 }}>
                <SafeAreaView>
                    <View style={{ marginTop: HEIGHT * 0.03 }}><HeaderComponent leftlogo={backarrow} title="MEMBERSHIP" leftOnPress={() => navigation.goBack()} /></View>
            ...
            <View style={{ margin: HEIGHT * 0.015, height: HEIGHT * 0.54 }}>
                ...
                <FlatList showsVerticalScrollIndicator={false} data={pointHistory} renderItem={({ item }) => <PointsHistory date={item.date} logo={item.logo} title={item.title} />} />
